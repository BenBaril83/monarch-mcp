...
**Transports**: `main()` picks a transport from `MCP_TRANSPORT` — `stdio` (default, for local MCP clients) or `http`/`streamable-http`. The HTTP path (`run_http_async()`/`build_http_app()`) serves the SDK's Streamable HTTP app over uvicorn, bound via `HOST`/`PORT` (default `127.0.0.1:8000`; the Docker image sets `0.0.0.0`). Since the built-in MCP auth machinery (`TokenVerifier`/`AuthSettings`) requires a real OAuth issuer, HTTP mode instead uses a small custom `BearerTokenAuthMiddleware` gated by `MCP_HTTP_AUTH_TOKEN` — set it whenever the server is reachable off-box; unset, HTTP mode logs a loud warning and serves unauthenticated. See `Dockerfile` / `docker-compose.yml` for the containerized deployment (session falls back to a file under `MONARCH_SESSION_DIR` since there's no OS keyring in a container).

The SDK's Streamable HTTP transport separately enforces its own DNS-rebinding protection (`mcp.settings.transport_security`, unrelated to `BearerTokenAuthMiddleware`) that only allows `Host: localhost`/`127.0.0.1`/`[::1]` by default — any other `Host` header (e.g. behind a reverse proxy / Cloudflare Tunnel) gets a `421 Invalid Host header`. `_configure_transport_security()` (called from `build_http_app()`) extends the default allowlist from `MCP_ALLOWED_HOSTS`/`MCP_ALLOWED_ORIGINS` (comma-separated, `host:*` wildcard-port syntax supported) rather than replacing it, so localhost access keeps working too.
